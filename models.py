...
class Player(db.Model):
    ...
    def active_admin_role(self):
        ...
        return admin_role.role if admin_role else None
    ...
    def visible_badges(self):
        ...
        from models import PlayerBadge
...
class GameMode(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), nullable=False, unique=True)  # bedwars, skywars, etc
    display_name = db.Column(db.String(100), nullable=False)
    ...
    # Relationships
    player_ratings = db.relationship('PlayerGameRating', backref='game_mode', lazy=True, cascade='all, delete-orphan')
    ...
    def create_default_modes(cls):
        ...
        default_modes = [
            ...
            {
                'name': 'skywars',
                'display_name': 'Sky Wars',
                'icon': 'fas fa-cloud',
                'color': '#3498db'
            },
            {
                'name': 'fireballfight',
                'display_name': 'Fireball Fight',
                'icon': 'fas fa-fire',
                'color': '#ff6b35'
            },
            {
                'name': 'tntrun',
                'display_name': 'TNT Run',
                'icon': 'fas fa-bomb',
                'color': '#f39c12'
            },
            {
...
class PlayerGameRating(db.Model):
    """Player ratings for different game modes in ASCEND system"""

    id = db.Column(db.Integer, primary_key=True)
    player_id = db.Column(db.Integer, db.ForeignKey('player.id'), nullable=False)
    game_mode_id = db.Column(db.Integer, db.ForeignKey('game_mode.id'), nullable=False)
    
    # Rating tiers (F, D, C, B, A, S, S+)
    kd_rating = db.Column(db.String(5), default='F')  # Kill/Death ratio rating
    kills_rating = db.Column(db.String(5), default='F')  # Kills rating
    objective_rating = db.Column(db.String(5), default='F')  # Beds/Frags/Objectives rating
    efficiency_rating = db.Column(db.String(5), default='F')  # Overall efficiency rating
    overall_rating = db.Column(db.String(5), default='F')  # Overall combined rating
    
    # Statistics for the specific mode
    mode_kills = db.Column(db.Integer, default=0)
    mode_deaths = db.Column(db.Integer, default=0)
    mode_objectives = db.Column(db.Integer, default=0)  # beds/frags/etc
    mode_games = db.Column(db.Integer, default=0)
    mode_wins = db.Column(db.Integer, default=0)
    mode_experience = db.Column(db.Integer, default=0)
    
    # Admin evaluation
    admin_notes = db.Column(db.Text, nullable=True)
    last_evaluated_by = db.Column(db.String(100), nullable=True)
    last_evaluated_at = db.Column(db.DateTime, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    # Relationships
    player = db.relationship('Player', backref='game_ratings')

    def __repr__(self):
        return f'<PlayerGameRating {self.player_id}:{self.game_mode_id}>'

    @property
    def mode_kd_ratio(self):
        """Calculate K/D ratio for this mode"""
        if self.mode_deaths == 0:
            return self.mode_kills if self.mode_kills > 0 else 0
        return round(self.mode_kills / self.mode_deaths, 2)

    @property
    def mode_win_rate(self):
        """Calculate win rate for this mode"""
        if self.mode_games == 0:
            return 0
        return round((self.mode_wins / self.mode_games) * 100, 1)
    ...
    def tier_color(self):
        ...
        tier_colors = {
            ...
            'B': '#ffd700',    # Gold
            'A': '#32cd32',    # Lime green
            'S': '#00bfff',    # Deep sky blue
            'S+': '#ff1493'    # Deep pink
        }
        return tier_colors.get(self.overall_rating, '#6c757d')

    def calculate_auto_ratings(self):
        """Auto-calculate ratings based on statistics"""
        # K/D Rating
        kd = self.mode_kd_ratio
        if kd >= 4.0:
            self.kd_rating = 'S+'
        elif kd >= 3.0:
            self.kd_rating = 'S'
        ...
        # Kills Rating
        if self.mode_kills >= 1000:
            self.kills_rating = 'S+'
        elif self.mode_kills >= 500:
            self.kills_rating = 'S'
        elif self.mode_kills >= 250:
            self.kills_rating = 'A'
        elif self.mode_kills >= 100:
            self.kills_rating = 'B'
        elif self.mode_kills >= 50:
            self.kills_rating = 'C'
        elif self.mode_kills >= 10:
            self.kills_rating = 'D'
        else:
            self.kills_rating = 'F'

        # Objective Rating (beds/frags/etc)
        if self.mode_objectives >= 200:
            self.objective_rating = 'S+'
        elif self.mode_objectives >= 100:
            self.objective_rating = 'S'
        elif self.mode_objectives >= 50:
            self.objective_rating = 'A'
        elif self.mode_objectives >= 25:
            self.objective_rating = 'B'
        elif self.mode_objectives >= 10:
            self.objective_rating = 'C'
        elif self.mode_objectives >= 5:
            self.objective_rating = 'D'
        else:
            self.objective_rating = 'F'

        # Efficiency Rating (based on win rate and activity)
        win_rate = self.mode_win_rate
        activity_score = min(100, self.mode_games / 10)  # Max score at 1000+ games
        
        efficiency_score = (win_rate + activity_score) / 2
        
        if efficiency_score >= 90:
            self.efficiency_rating = 'S+'
        elif efficiency_score >= 80:
            self.efficiency_rating = 'S'
        elif efficiency_score >= 70:
            self.efficiency_rating = 'A'
        elif efficiency_score >= 60:
            self.efficiency_rating = 'B'
        elif efficiency_score >= 50:
            self.efficiency_rating = 'C'
        elif efficiency_score >= 30:
            self.efficiency_rating = 'D'
        else:
            self.efficiency_rating = 'F'

        # Overall Rating (average of all ratings)
        ratings = [self.kd_rating, self.kills_rating, self.objective_rating, self.efficiency_rating]
        tier_values = {'F': 0, 'D': 1, 'C': 2, 'B': 3, 'A': 4, 'S': 5, 'S+': 6}
        avg_value = sum(tier_values.get(r, 0) for r in ratings) / len(ratings)
        
        if avg_value >= 5.5:
            self.overall_rating = 'S+'
        elif avg_value >= 4.5:
            self.overall_rating = 'S'
        elif avg_value >= 3.5:
            ...
            self.overall_rating = 'F'

    @classmethod
    def get_or_create_rating(cls, player_id, game_mode_id):
        """Get existing rating or create new one"""
        rating = cls.query.filter_by(player_id=player_id, game_mode_id=game_mode_id).first()
        if not rating:
            rating = cls(player_id=player_id, game_mode_id=game_mode_id)
