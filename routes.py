...
from models import Player, Quest, PlayerQuest, Achievement, PlayerAchievement, CustomTitle, PlayerTitle, GradientTheme, PlayerGradientSetting, SiteTheme, ShopItem, ShopPurchase, Clan, ClanMember, Tournament, TournamentParticipant, PlayerActiveBooster, AdminCustomRole, PlayerAdminRole, Badge, PlayerBadge, ReputationLog
import os
import csv
import io
...
def player_profile(player_id):
    ...
    # Initialize game modes if they don't exist
    from models import GameMode
    if GameMode.query.count() == 0:
        GameMode.create_default_modes()
    
    # Get all active game modes and convert to dictionaries for JSON serialization
...
def create_player_rating(player_id, mode_id):
    ...
    try:
        ...
        # Create rating with basic data from player's main stats
        player = Player.query.get_or_404(player_id)
        
        rating = PlayerGameRating(
            player_id=player_id,
            game_mode_id=mode_id,
            mode_kills=player.kills // 3,  # Distribute main stats
            mode_deaths=player.deaths // 3,
            mode_objectives=player.beds_broken // 3,
            mode_games=player.games_played // 3,
            mode_wins=player.wins // 3,
            mode_experience=player.experience // 3
        )
...
def shop():
    ...
    shop_items.extend([
        ShopItem(
            name='custom_role_purchase',
            display_name='Покупка кастомной роли',
            description='Позволяет установить свою роль в профиле',
            category='cosmetic',
            price_coins=50000,
            ...
            item_data='{"purchasable_role": true}'
        ),
        ShopItem(
            name='profile_banner_upgrade',
            display_name='Улучшение баннера профиля',
            description='Позволяет кастомизировать баннер профиля',
            category='cosmetic',
            price_coins=25000,
            ...
            rarity='rare',
            icon='fas fa-image',
            item_data='{"banner_upgrade": true}'
        ),
        ShopItem(
            name='xp_booster_2x_1h',
            display_name='Бустер опыта (x2, 1 час)',
            description='Увеличивает получаемый опыт в 2 раза на 1 час',
            category='active_booster',
            price_coins=5000,
            price_reputation=50,
            unlock_level=5,
            rarity='uncommon',
            icon='fas fa-bolt',
            item_data='{"booster_type": "xp", "duration_hours": 1, "multiplier": 2.0}'
        ),
        ShopItem(
            name='coins_booster_1.5x_2h',
            display_name='Бустер койнов (x1.5, 2 часа)',
            description='Увеличивает получаемые койны в 1.5 раза на 2 часа',
            category='active_booster',
            price_coins=7500,
            price_reputation=75,
            unlock_level=15,
            rarity='rare',
            icon='fas fa-coins',
            item_data='{"booster_type": "coins", "duration_hours": 2, "multiplier": 1.5}'
        ),
        ShopItem(
            name='reputation_booster_1.5x_1h',
            display_name='Бустер репутации (x1.5, 1 час)',
            description='Увеличивает получаемую репутацию в 1.5 раза на 1 час',
            category='active_booster',
            price_coins=6000,
            ...
            rarity='rare',
            icon='fas fa-star',
            item_data='{"booster_type": "reputation", "duration_hours": 1, "multiplier": 1.5}'
        ),
        ShopItem(
            name='cosmetic_title_legend',
            display_name='Косметический титул "Легенда"',
            description='Уникальный титул для вашего профиля',
            category='cosmetic',
            price_coins=100000,
        ...
        ShopItem(
            ...
            description='Добавляет 2 слота для кастомных эмодзи к роли',
            category='emoji_slot',
            price_coins=18000,
            price_reputation=350,
            unlock_level=20,
            rarity='rare',
        ...
        ShopItem(
            name='profile_border_gold',
            display_name='Золотая рамка профиля',
            description='Золотая рамка для вашего аватара в профиле',
            category='cosmetic',
            price_coins=15000,
            price_reputation=250,
            unlock_level=30,
            rarity='rare',
            icon='fas fa-border-all',
            item_data='{"profile_border": "gold"}'
        ),
        ShopItem(
            name='animated_avatar_effect',
            display_name='Эффект анимированного аватара',
            description='Добавляет эффект анимации к вашему аватару',
            category='cosmetic',
            price_coins=30000,
            price_reputation=400,
            unlock_level=40,
            rarity='epic',
            icon='fas fa-film',
            item_data='{"avatar_effect": "animated"}'
        )
    ])
    ...
    try:
        ...
        if item.category == 'booster':
            if item.name.startswith('coin'):
                bonus_coins = int(item.item_data.split('"bonus_amount": ')[1].split('}')[0]) if item.item_data else 1000
                player.coins += bonus_coins
                flash(f'Вы получили {bonus_coins} койнов!', 'success')
            elif item.name.startswith('reputation'):
                bonus_rep = int(item.item_data.split('"bonus_amount": ')[1].split('}')[0]) if item.item_data else 100
                player.reputation += bonus_rep
                flash(f'Вы получили {bonus_rep} репутации!', 'success')
            elif item.name.startswith('xp'):
                bonus_xp = int(item.item_data.split('"bonus_amount": ')[1].split('}')[0]) if item.item_data else 1000
                player.experience += bonus_xp
                flash(f'Вы получили {bonus_xp} опыта!', 'success')
        # Обработка активных бустеров
        elif item.category == 'active_booster':
            ...
            try:
                ...
                duration = data.get('duration_hours', 1) * 3600
            ...
            flash(f'Активирован бустер x{multiplier} на {duration_text}!', 'success')
        else:
...
def my_profile():
    ...
    # Get player's badges and their visibility settings
    player_badges_query = PlayerBadge.query.filter_by(player_id=player.id).all()
    player_badges_data = {}
    for pb in player_badges_query:
        badge = Badge.query.get(pb.badge_id)
        if badge and badge.is_active:
            player_badges_data[badge.id] = {
                'badge': badge,
    ...
    return render_template('my_profile.html',
                         ...
                         player_badges=player_badges_data)
...
def create_badge():
    ...
    try:
        ...
        emoji_url = request.form.get('emoji_url', '').strip() or None
...
def admin_create_role():
    ...
    try:
        ...
        color = request.form.get('role_color', '#ffd700')
        emoji = request.form.get('role_emoji', '').strip()
        emoji_class = request.form.get('role_emoji_class', '').strip()
        is_visible = request.form.get('role_visible') == 'on'
        has_gradient = request.form.get('role_gradient') == 'on'
        gradient_end_color = request.form.get('role_gradient_end', '#ffaa00') if has_gradient else None
        ...
        if 'emoji_file' in request.files:
            ...
            if file and file.filename:
                # Validate file using extension only
                allowed_extensions = {'png', 'svg', 'webp', 'gif'}

                filename = secure_filename(file.filename)
                file_ext = filename.rsplit('.', 1)[1].lower() if '.' in filename else ''

                if file_ext not in allowed_extensions:
                    flash('Недопустимый формат файла! Разрешены: PNG, SVG, WEBP, GIF', 'error')
                    return redirect(url_for('admin_roles'))

                # Check file size (512KB max for GIF, 200KB for others)
                file.seek(0, os.SEEK_END)
                file_size = file.tell()
                file.seek(0)

                max_size = 512 * 1024 if file_ext == 'gif' else 200 * 1024
                if file_size > max_size:
                    max_size_mb = max_size / (1024 * 1024)
                    flash(f'Файл слишком большой! Максимальный размер: {max_size_mb:.1f}MB', 'error')
                    return redirect(url_for('admin_roles'))
                ...
                emoji_path = os.path.join(emoji_dir, unique_filename)

                # Save file
                file.save(emoji_path)
                emoji_url = f"/static/emojis/{unique_filename}"
...
def admin_assign_role():
    """Assign role to player (admin only)"""
    if not session.get('is_admin', False):
        flash('Доступ запрещен!', 'error')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Remove any existing role assignment for this player
        ...
        flash(f'Роль "{role.name}" присвоена игроку {player.nickname}!', 'success')

    except Exception as e:
        app.logger.error(f"Error assigning role: {e}")
        flash('Ошибка при присвоении роли!', 'error')

    return redirect(url_for('admin_roles'))

@app.route('/admin/edit_role/<int:role_id>', methods=['POST'])
def admin_edit_role(role_id):
    """Edit custom role (admin only)"""
    if not session.get('is_admin', False):
        flash('Доступ запрещен!', 'error')
        return redirect(url_for('login'))
    ...
    try:
        ...
        role_color = request.form.get('role_color', '#ffd700')
        role_emoji = request.form.get('role_emoji', '').strip()
        role_emoji_class = request.form.get('role_emoji_class', '').strip()
        role_visible = request.form.get('role_visible') == 'on'
        ...
        existing = AdminCustomRole.query.filter(
            AdminCustomRole.name == role_name,
            AdminCustomRole.id != role_id
        ).first()
        ...
        role.emoji = role_emoji if role_emoji else None
        role.emoji_class = role_emoji_class if role_emoji_class else None
        role.is_visible = role_visible

        db.session.commit()
        flash(f'Роль "{role_name}" успешно обновлена!', 'success')

    except Exception as e:
        app.logger.error(f"Error editing role: {e}")
        flash('Ошибка при редактировании роли!', 'error')
...
def admin_delete_role(role_id):
    ...
    try:
        ...
        flash(f'Роль "{role_name}" удалена!', 'success')

    except Exception as e:
        app.logger.error(f"Error deleting role: {e}")
        flash('Ошибка при удалении роли!', 'error')
